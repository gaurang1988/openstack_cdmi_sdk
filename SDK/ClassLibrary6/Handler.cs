...
namespace ObjectStoreSDK
{
    ...
    public class Handler
    {
        ...
        /// <returns>True if created successfully
        /// False otherwise</returns>
        public Boolean createContainer(String containerName, String metadataKey,String metadataValue)
        // 
        {
            try
            {
                ...
                byte[] buf = Encoding.UTF8.GetBytes(
                     "{ \"metadata\" : {" +" \"cTime\":\""+DateTime.Now.ToString()+ "\" , \""+ metadataKey + "\"  : \"" +
                    metadataValue + "\"} }");
        ...
        public List<String> displayAccountContents()
        {
            ...
            ExecutionResult executionResult = request.executeRequest();
            List<String> contents = JSONParser.getChildren
                (JSONParser.getFieldValue("children",executionResult.responseFromServer));
            return contents;
        }
       ...
       /// <param name="metadataValue">Metadata Value</param>
        ...
        /// False otherwise</returns>
        public Boolean createObject(String containerName, String objectName,
           String filePath, String metadataKey,String metadataValue)
        {
            try
            {
                path = CommonConstants.CDMI_PATH_CONSTANT + client.username +
                ...
                byte[] buf = Encoding.UTF8.GetBytes(
                    "{ \"mimetype\" : \"text/plain\"," + "\"value\" : \"" +
                    objectValue + "\" ,\"metadata\" : {" +" \"cTime\":\""+DateTime.Now.ToString()
                    + "\" , \""+ metadataKey + "\"  : \"" +
                    metadataValue + "\"} }");
                PutHTTPRequest request = new PutHTTPRequest(this.client);
                request.createHTTPRequest(path, Resource1.ObjectRequest);
        ...
        public Boolean deleteContainer(String containerName)
        {
            try
            {
                path = CommonConstants.CDMI_PATH_CONSTANT + client.username
                    + "/" + containerName;
                List<String> containerList = displayAccountContents();
